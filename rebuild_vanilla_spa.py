...
def rebuild_spa():
    ...
    # We will extract the style block
    style_match = re.search(r'<style>(.*?)</style>', old_html, re.DOTALL)
    styles = style_match.group(1) if style_match else ""

    # We will extract the JS utilities block
    # Actually, we will just write a new HTML file combining the best parts.
...
<body>
  ...
  <script>
    ...
    // RENDER COMPONENTS
    function playCardFree(p, gen, ts){
        const isB=p.type==='AGGRESSIVE'||p.type==='MOMENTUM',isC=p.type==='CONTRARIAN'||p.type==='SQUEEZE';
        const cls=isB?'bull':isC?'neut':'';
        return`<div class="play-card"><div class="play-tag ${cls}">${esc(p.type)}</div><div class="play-th">${esc(p.thesis)}</div><div class="play-det">${esc(p.details)}</div></div>`;
    }
    ...
    async function loadTerminalPayload() {
      try {
        ...
        document.getElementById('vitals').innerHTML = [
            `<div class="vital"><div class="vital-val ${btcCls}">${esc(m.btcPrice||'—')}</div><div class="vital-lbl">BTC/USD</div><div class="vital-sub">24h ${num(m.btcChange).toFixed(2)}%</div></div>`,
            `<div class="vital"><div class="vital-val ${scls}">${esc(signal)}</div><div class="vital-lbl">Signal</div><div class="vital-sub">AI · ${b.active_regime||'—'}</div></div>`,
            `<div class="vital"><div class="vital-val neut">${b.confidence||0}%</div><div class="vital-lbl">Confidence</div><div class="vital-sub">${(b.forces||[]).length} forces</div></div>`
        ].join('');
        ...
        document.getElementById('sig-row').innerHTML=`<span class="sig ${scls}">${esc(signal)}</span><span class="ts-badge">${esc(b.time_sensitivity||'THIS WEEK')}</span>${b.active_regime?`<span class="regime-tag">${esc(b.active_regime)}</span>`:''}<span class="conf">${b.confidence||0}% AI CONFIDENCE</span>`;
        document.getElementById('updated-line').textContent=`LIVE DATA · ${m.label||''}`;

        // Narrative
        document.getElementById('story-intro').textContent = b.eli5 || "Markets are nervous.";
        document.getElementById('chaos-text').innerHTML=`&ldquo;${esc(b.chaos_line||'')}&rdquo;`;

        // Intelligence Tabs
        document.getElementById('text-eli5').innerText = b.eli5 || "--";
        document.getElementById('text-analyst').innerText = b.analyst || "--";
        document.getElementById('text-quant').innerText = b.quant || "--";

        // Macro Forces
        const allForces = ["MONEY", "TECH", "ENERGY", "POLITICS", "WAR", "DEBT", "JOBS", "FOOD", "PEOPLE"];
        const activeForces = b.forces || ["MONEY"];
        ...
        document.getElementById('agent-panel-grid').innerHTML = ['fundamental','technical','sentiment'].map(ag => {
            const agent = a[ag] || {};
            const bias = agent.bias || 'NEUTRAL';
            const bcls = bias==='BULLISH'?'bull':bias==='BEARISH'?'bear':'neut';
            return `<div class="agent-box"><div class="ag-n">${ag.toUpperCase()}</div><div class="ag-bias ${bcls}">${bias} / ${agent.confidence||0}%</div><div class="ag-t">${esc(agent.thesis||'--')}</div></div>`;
        }).join('');
        ...
        // Plays
        document.getElementById('plays-grid').innerHTML = (b.plays||[]).map(p=>playCardFree(p)).join('');

        // Trap / Edge
        document.getElementById('trap-text').textContent = b.trap || "--";
        document.getElementById('edge-text').textContent = b.edge || "--";

        // Ledger
        document.getElementById('track-body').innerHTML = `
            <div class="track-row"><span class="track-date">2026-05-28</span><span class="track-hl">Fear Rises, Vol Stays Contained</span><span class="track-result open">⏳ VOLATILE / OPEN</span></div>
        `;

        // Vitals Context Grid
        document.getElementById('context-grid').innerHTML = [
            {l:"10Y Treasury", v:`${m.yieldVal||'—'}%`, s:`Prev ${m.yieldPrev||'—'}%`},
            {l:"DXY Dollar", v:m.dxyVal||'—', s:"Neutral"},
            {l:"VIX", v:m.vixVal||'—', s:"Implied Volatility"},
            {l:"S&P 500", v:m.spxVal||'—', s:m.spxChange||'—'},
            {l:"BTC DOM", v:`${m.btcDominance||'—'}%`, s:m.totalMarketCap||'—'},
        ...
        // Wires
        document.getElementById('wires-list').innerHTML = `<div style="margin-bottom: 8px;">// ${esc(m.newsText||'--')}</div>`;
