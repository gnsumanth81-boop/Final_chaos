function render(latest,signals){
      ...
      if (hSig) {
        hSig.className = 'hm-val ' + scls;
        hSig.innerHTML = esc(signal) + ' <span id="hdr-conf" style="font-size: 0.8em; opacity: 0.8; margin-left: 4px;">' + esc(b.confidence||0) + '%</span>';
      }
      const hBtc = document.getElementById('hdr-btc');
      if (hBtc) {
        hBtc.textContent = esc(m.btcPrice || '--');
        hBtc.className = 'hm-val ' + btcCls;
      }
      const hFear = document.getElementById('hdr-fear');
      if (hFear) {
        hFear.textContent = esc(m.fearIndex || '--');
        hFear.className = 'hm-val ' + fearCls;
      }
      const hDxy = document.getElementById('hdr-dxy');
      if (hDxy) {
        hDxy.textContent = esc(m.dxyVal || '--');
      }
      const h10y = document.getElementById('hdr-10y');
      if (h10y) {
        h10y.textContent = (m.yieldVal || '--') + '%';
