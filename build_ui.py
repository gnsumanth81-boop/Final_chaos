...
html = re.sub(r'(\d+\.\d+)rem', scale_rem, html)
html = re.sub(r'(\.\d+)rem', lambda m: scale_rem(re.match(r'(\.\d+)', m.group(1))), html)
...
css_add = '''
    ...
    .mobile-nav a{color:var(--fg);text-decoration:none;font-size:1.5rem;font-family:var(--font-mono);text-transform:uppercase;letter-spacing:2px}
    ...
    .desktop-nav a{color:var(--muted);text-decoration:none;font-family:var(--font-mono);font-size:0.85rem;text-transform:uppercase;letter-spacing:1px;transition:0.2s}
...
# 5. AUDIO PERSONALITIES
audio_js = '''function chaosSpeak(paneId,btn){
    ...
    u.lang='en-US';
    // Audio Personalities
    if(paneId.includes('eli5')){ u.rate=1.1; u.pitch=1.2; } // Friendly, faster
    else if(paneId.includes('quant')){ u.rate=0.9; u.pitch=0.7; } // Deep, authoritative
    else { u.rate=1.0; u.pitch=1.0; } // Analyst: Neutral
    btn.classList.add('playing');btn.textContent='PLAYING';
    u.onend=()=>{btn.classList.remove('playing');btn.textContent='LISTEN'};
    speechSynthesis.cancel();speechSynthesis.speak(u);
}'''
audio_js = audio_js.replace('\\\\', '\\') # if any escapes
html = re.sub(r'function chaosSpeak\(paneId,btn\)\{.*?\}', lambda m: audio_js, html, flags=re.DOTALL)


with open('public/index.html', 'w', encoding='utf-8') as f:
    f.write(html)
print("Rewrote index.html with all requested features")
